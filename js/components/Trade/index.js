...
export default class Trade extends Component {
  ...
  componentDidMount() {
    this.props.updatePartnerActivity({
      [this.partner.id]: this.partner.online
    });
  ...
  onMessage = event => {
    ...
    switch (data.type) {
      ...
      default:
        ...
        if (message.messages) {
          this.setState({
            messages: message.messages.map(message => keysToCamelCase(message))
          });
  ...
  isUserBuying = () => {
    return (
      this.isTradeLoaded() &&
      tradeType(this.props.trade, this.props.user.id) === tradeTypeBuy
    );
  };
  ...
  get partner() {
    return this.isTradeLoaded()
      ? tradePartner(this.props.trade, this.props.user.id)
      : "";
  }
